...
bool CGateClientMgr::OnMessage( RakNet::Packet* pData )
{
	...
	if ( pMsg->cSysIdentifer == ID_MSG_VERIFY )
	{
		...
		{
			CLogMgr::SharedLogMgr()->PrintLog("a Client connected ip = %s",pData->systemAddress.ToString(true)) ;
	...
	if ( ID_MSG_C2LOGIN == pMsg->cSysIdentifer || ID_MSG_C2GAME == pMsg->cSysIdentifer ) // send to Login Server  // send to game server 
	{
		...
		if ( CheckServerStateOk(pDstClient,ID_MSG_C2LOGIN == pMsg->cSysIdentifer ? eServer_Login: eServer_Game ) == false )
		{
			return true ;
		}
	...
	else if ( MSG_TRANSER_DATA == pMsg->usMsgType ) // Distribute to client ;
	{
		...
		if ( pDstClient == NULL )
		{
			CLogMgr::SharedLogMgr()->ErrorLog("can not send message to client , client is NULL") ;
			return true ;
		}
...
void CGateClientMgr::OnClientWaitReconnectTimeUp(stGateClient* pClient )
{
	...
	msg.nSeesionID = pClient->nSessionId ;
...
void CGateClientMgr::OnPeerDisconnected(RakNet::RakNetGUID& nPeerDisconnected, RakNet::Packet* pData )
{
	...
	CLogMgr::SharedLogMgr()->PrintLog("not verify peer disconnected ip = %s",pData->systemAddress.ToString(true) ) ;
}
...
bool CGateClientMgr::CheckServerStateOk( stGateClient* pClient, eServerType eCheckType )
{
	...
	if ( (eCheckType == eServer_Login || eCheckType == eServer_Max ) && m_nLoginServerNetWorkID == RakNet::UNASSIGNED_RAKNET_GUID )  // login server disconnect ;
	{
		
		msg.nServerType = eServer_Login ;
		CGateServer::SharedGateServer()->GetNetWork()->SendMsg((char*)&msg,sizeof(msg),pClient->nNetWorkID,false) ;
		return false;
	}
	
	if ( (eCheckType == eServer_Game || eCheckType == eServer_Max )  && RakNet::UNASSIGNED_RAKNET_GUID == m_nGameServerNetWorkID ) // game server disconnect 
	{
		msg.nServerType = eServer_Game ;
		CGateServer::SharedGateServer()->GetNetWork()->SendMsg((char*)&msg,sizeof(msg),pClient->nNetWorkID,false) ;
		return false ;
	}
